...
DNa02=read.neurons.catmaid(12526673)
```
First, grouping the presynaptic inputs by region, we get: 
```{r, echo=TRUE}
pre_neuropils = inside_neuropils(DNa02,brain = FAFB14NP.surf, "PRE")
pre_neuropils
```
Next,grouping the postsyanptic inputs by region, we get:
```{r, echo=TRUE}
post_neuropils= inside_neuropils(DNa02,brain = FAFB14NP.surf, "POST")
post_neuropils
```
...
all_neuropils_df=merge(post_neuropils_df,pre_neuropils_df,by='names',all=TRUE)
...
```
<img src="images/A2_connections_grouped.png"/>
...
neuron.heatmap(connectivity_matrix)
```
